...
def run(context):
    ...
    try:
        ...
        # Give the old process a tiny moment to exit its TCP connection cleanly before stopping
        import time
        time.sleep(1)
...
"""
    ...
    # Execute the script in Fusion synchronously or asynchronously.
    # App.executeTextCommand runs it in the active environment.
    # To run a Python script file by path, we can try firing it via the TextCommand palette
    try:
        # We can add the script dynamically or just execute the python file 
        # Actually, running a generic python file via text commands:
        app.executeTextCommand(f'TextCommands.RunPythonScript "{script_path}"')
