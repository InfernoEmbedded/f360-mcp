...
@pytest.mark.asyncio
async def test_create_sketch(mock_fusion, recorded_commands):
    # 1. Create a sketch
    result = await create_sketch(name="TestSketch", plane_name="XY")
    assert "Successfully" in result["message"]
    assert result["sketch_name"] == "TestSketch"
    
    # 2. Check stability
    compare_command_logs("test_create_sketch", recorded_commands)
